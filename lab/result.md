...
- 假设：继承当前任务“退化条件证据分配改善原始紧致 OBB 检测”的假设，本节不提出新科学目标。
- 代码与配置：2026-09-16 B 工程核验；科学组件最后修改提交为 9f48af8a5d236175e9fecf3357b53bf87b0c2ef9，ai4rs 为 6095d3a3155570324e9e01c88cd2e4a3b55ae7c3。从保留 checkpoint 的 meta.cfg 原样提取 configs/r005_legacy_hrsc_B2_seed17.py、configs/r005_legacy_hrsc_M1_seed17.py、configs/r005_legacy_dota_B2_seed17.py；三个配置正文 SHA-256 及各组保留末态 SHA-256 见 configs/r005_checkpoint_rebuild.json。恢复入口为 src/rebuild_r005_intermediates.py，工程说明见 doc/checkpoint_and_gpu_execution.md。
- 基线与主要指标：本节只有维护统计，没有读取新的确认指标。2026-09-16 定向核验，HRSC/B2 seed17 的 epoch1–34 为 34 个文件、5,080,127,958 bytes；HRSC/M1 seed17 的 epoch1–35 为 35 个、5,231,673,681 bytes；DOTA/B2 seed17 的 epoch1–34 为 34 个、6,138,662,422 bytes。合计 103 个独立 inode、16,450,464,061 bytes。三组分别保留 epoch35、epoch36、epoch35；这三份末态含 optimizer、两个 scheduler、EMA、epoch/iter/message_hub，且结果 JSON 正在引用它们。
- 结论：**本任务未完成；本节只记录已完成训练片段与 checkpoint 保留、重建事实，不代表科学验收或 continue/stop 裁决。** 103 个中间 checkpoint 没有现有结果引用，按精确恢复清单和零进程引用核验可清理；实际删除前必须先提交推送本节、全部实际配置及恢复入口。取消种子的既有产物、正在运行的 DOTA/M1、unsharded 遗留、全部 best、latest/final 和明确下游依赖均保留。
